...
\begin{align}\tag{4}\label{SS}
SST &= \sum_i^N (y - \bar{y})^2 = y'y \\
SSR &= \sum_i^N (X\hat{\beta} - \bar{y})^2 = \hat{y}'\hat{y} \\
SSE &= \sum_i^N (y - \hat{y})^2 = e'e,
\end{align}where $\hat{y} \equiv X\hat{\beta}$. Then $R^2$ is defined as the ratio of the regression sum of squares to the total sum of squares:
\begin{align}\tag{5}\label{R2}
R^2 \equiv \frac{SSR}{SST} = 1 - \frac{SSE}{SST}
\end{align}$R^2$ ranges between 0 and 1, with 1 being a perfect fit. According to (\ref{cov}), the variance of a single coefficient $\hat{\beta}_k$ is proportional to the quantity $(X'X)_{kk}^{-1}$, where $k$ denotes the kth diagonal element of $(X'X)^{-1}$, and can be rewritten as
\begin{align}\tag{6}\label{cov2}
var(\hat{\beta}_k) &= \sigma^2 (X'X)_{kk}^{-1} \\ &= \frac{\sigma^2}{(1 - R_k^2)\sum_i^N (x_{ik} - \bar{x}_k)^2},
\end{align}where $R_k^2$ is the $R^2$ in the regression of the kth variable, $x_k$, against the other predictors [[A1]](#A1).
...
\end{align} To be explicit, a single coefficient, $\hat{\beta}_k$, is distributed as
\begin{align}\tag{8}
...
\begin{align}\tag{10}\label{tstat}
t_{\hat{\beta}_k} = \frac{\hat{\beta}_k - \beta_k}{s(\hat{\beta}_k)} \sim t_{n-K},
\end{align} where $s(\hat{\beta}_k)$ is the standard error of $\hat{\beta}_k$
\begin{align}\tag{11}
s(\hat{\beta}_k)^2 = \hat{\sigma}^2 (X'X)_{kk}^{-1},
\end{align} and $\hat{\sigma}^2$ is the unbiased estimator of $\sigma^2$
\begin{align}\tag{12}
...
\end{align} where $\Delta = t_{1-\alpha/2, n-K} s(\hat{\beta}_k)$, and $t_{1-\alpha/2, n-K}$ is the $\alpha/2$-level critical value for the t-distribution with $n-K$ degrees of freedom.
...
\end{align}We reject the null-hypothesis if $P[t] < \alpha$.
...
\begin{align}\tag{16}\label{F2}
F = \frac{(N-K) R^2}{(K-1) (1-R^2)}
\end{align}Notice how, for fixed $R^2$, the F-statistic decreases with an increasing number of predictors $K$. Adding uninformative predictors to the model will decrease the significance of the regression, which motivates parsimony in constructing linear models.
...
\end{align} The last line is by properties of [affine transformations on multivariate normal distributions](https://en.wikipedia.org/wiki/Multivariate_normal_distribution#Affine_transformation).
